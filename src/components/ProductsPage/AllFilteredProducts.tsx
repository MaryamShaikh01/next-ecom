                ...
                isGrid ? (
                        ...
                        {data.map((item) =>
                            <div key={item.id} className='flex flex-col p-5 border-r-1 border-r-[var(--border)]'>
                                <div className="relative ">
                                    <img src={item.image} alt={item.productTitle} className="w-[200px] h-[200px] object-cover place-self-center" />
                                ...
                                <div className=" flex flex-col justify-center gap-y-1">
                                    <p className='bg-gradient-to-r from-lime-300  to-green-300 text-[10px] font-extrabold w-fit rounded-[14px] px-2.5 py-1 text-[var(--badgeGreenText)] uppercase'>{item.badgeText}</p>
                    ...
                    : (
                        <>
                            ...
                            <div className='order-1 border-[var(--border)] rounded-[12px] grid grid-cols-1 lg:grid-cols-2 min-[2600px]:grid-cols-3 pb-10 border-1 mb-20'>
                                {data.map((item) =>
                                    <div key={item.id} className='flex p-5 border-r-1 border-r-[var(--border)]'>
                                        <div className="relative ">
                                            <img src={item.image} alt={item.productTitle} className="min-w-[200px] min-h-[200px] object-cover place-self-center" />
                                            <div className='absolute top-0 left-0 flex justify-between items-center w-full'>
                                                <span className='rounded-3xl bg-[var(--accent)] px-2 py-1 text-[10px] font-extrabold text-white'>{item.discountPercentage}</span>
                                                <LikeHeart></LikeHeart>
                                            </div>
                                        ...
                                        <div className=" flex flex-col justify-center gap-y-1">
                                            ...
                                            <button type='button' className='w-fit border-1 border-[var(--primary)] rounded-3xl px-5 py-1 text-[var(--primary)] flex gap-3 items-center justify-between'>Add to cart <img src="/plus-icon.svg" alt="plus icon" /></button>
                                        </div>
